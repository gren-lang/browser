    ...
    function (sendToApp, initialModel) {
      ...
      return _Browser_makeAnimator(initialModel, function (model) {
        var nextNode = A2(
          __VirtualDom_map,
          __Main_UserMsg,
          view(__Main_getUserModel(model))
        );
        ...
        cornerNode = __VirtualDom_applyPatches(
          cornerNode,
          cornerCurr,
          cornerPatches,
          sendToApp
...
var _Debugger_document = F4(function (impl, flagDecoder, debugMetadata, args) {
  return __Platform_initialize(
    ...
    function (sendToApp, initialModel) {
      var divertHrefToApp =
        impl.__$setup &&
        impl.__$setup(function (x) {
          return sendToApp(__Main_UserMsg(x));
        });
      var view = impl.__$view;
      var title = __VirtualDom_doc.title;
...
function _Debugger_openWindow(popout) {
  ...
  doc.addEventListener("keydown", function (event) {
    ...
    event.key === "ArrowUp" &&
      (popout.__sendToApp(__Main_Up), event.preventDefault());
    event.key === "ArrowDown" &&
      (popout.__sendToApp(__Main_Down), event.preventDefault());
  });
...
function _Debugger_init(value) {
  if (typeof value === "boolean") {
    return A3(
      __Expando_Constructor,
      __Maybe_Just(value ? "True" : "False"),
      true,
      []
    );
  }
  ...
  if (typeof value === "object" && "$" in value) {
    ...
    if (char === 35 || (65 <= char && char <= 90)) {
      ...
      return A3(
        __Expando_Constructor,
        char === 35 ? __Maybe_Nothing : __Maybe_Just(tag),
        true,
        __Array_reverse(list)
      );
    }
...
var _Debugger_initKeyValueCons = F3(function (key, value, list) {
  return [
    { __$key: _Debugger_init(key), __$value: _Debugger_init(value) },
  ].concat(list);
});
